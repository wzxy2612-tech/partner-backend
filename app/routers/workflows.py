...
def register_connector(body: ConnectorBody,
    ...
    with session_for_principal(principal) as db:
        ...
        cid, cstatus = connectors.register_connector(
            db, principal.partner_id, body.kind, body.config)
    # Report the persisted row, not an assumption about it. This used to return
    # a hardcoded "unverified" alongside a freshly minted UUID that, on the
    # re-registration path, named no row at all.
    return {"id": str(cid), "kind": body.kind, "status": cstatus}
...
def clone_workflow(body: CloneBody, principal: Principal = Depends(require_partner)) -> dict:
    with session_for_principal(principal) as db:
        ...
        try:
            ...
            # Absent, or present in another tenant and hidden by RLS. Both are
            # 404 to the caller -- deliberately indistinguishable, so the
